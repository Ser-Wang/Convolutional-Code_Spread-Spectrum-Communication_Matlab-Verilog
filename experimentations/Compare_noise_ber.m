...
% Noise
for i_ebno = 1 : length(EbNo_dB)

    noisePower = 1 / (2 * codeRate * 10^(EbNo_dB(i_ebno)/10)); 
    ...
    y1 = awgn(x_ask_100,EbNo_dB(i_ebno),'measured');
    y2 = awgn(x_ask_80,EbNo_dB(i_ebno),'measured');
    y3 = awgn(x_ask_30,EbNo_dB(i_ebno),'measured');
    ...
    de_y2 = (y1 > 0.6);
    de_y3 = (y1 > 0.85);
    dec_y1 = vitdec(de_y1, trellis, 5*constraintLength, 'trunc', 'hard');
    dec_y2 = vitdec(de_y2, trellis, 5*constraintLength, 'trunc', 'hard');
    dec_y3 = vitdec(de_y3, trellis, 5*constraintLength, 'trunc', 'hard');
    [~ , ber_y1(i_ebno)] = biterr(info_bits, dec_y1);
    [~ , ber_y2(i_ebno)] = biterr(info_bits, dec_y2);
    [~ , ber_y3(i_ebno)] = biterr(info_bits, dec_y3);
...
% semilogy(EbNo_dB, ber_y3, 'o-', EbNo_dB, ber_y2, 's-', EbNo_dB, ber_y1, '^-');
% semilogy(EbNo_dB, ber_ask_30, 'o-', EbNo_dB, ber_ask_80, 's-', EbNo_dB, ber_ask_100, '^-', EbNo_dB, ber_y3, 'o-', EbNo_dB, ber_y2, 's-', EbNo_dB, ber_y1, '^-');
...
% subplot(4,1,4);
% plot(noise2); title('带噪声的 ASK 调制信号');
