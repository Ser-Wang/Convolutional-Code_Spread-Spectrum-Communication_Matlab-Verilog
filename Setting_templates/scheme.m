c = 1;
legends{c} = ' man, 30% ASK ';   % 为保证表格打印效果，字符最好左右留空格
L_lengths(c) = 256;                         % 基带信号码长
encode_type(c) = 1;                         % 编码类型，0:none, 1:man, 2:man+conv, 3:conv, 4:conv+dsss.
modulation_cell(c,1:2) = {'ask', 0.3};        % 调制类型('ASK' 'BPSK'可选); ASK调制深度(仅ASK时有效), ranges [0,1].
...
c = 2;
legends{c} = ' man, 100% ASK ';   % 为保证表格打印效果，字符最好左右留空格
L_lengths(c) = 256;                         % 基带信号码长
encode_type(c) = 1;                         % 编码类型，0:none, 1:man, 2:man+conv, 3:conv, 4:conv+dsss.
...
% c = 2;
% legends{c} = ' conv(2,1,9), g=[561 753] ';
% L_lengths(c) = 256;
% encode_type(c) = 3;
